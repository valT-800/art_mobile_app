...
export default function EditCompetitionScreen({route, navigation:{navigate, setOptions}}){
  ...
  const{id, title, description,start_time,end_time} = route.params;
    ...
    async function updateCompetition(title, description, start_time, end_time, publish){
      console.log('Publish',publish, {title, description, start_time, end_time})
        await api.put(`api/gallery/competitions/${id}`, {title, description,start_time, end_time,publish}).then(response => { 
        console.log('Competition',response.data)
        navigate('Competition', {id:id})
        }).catch(error => {
    ...
    const handlePublish=()=>{
      Alert.alert('Publish', 'Do you want to publish competition?', [
        {
          text: 'No, leave as draft',
          onPress: () => updateCompetition(newTitle,newDescription,format(startTime,'yyyy-MM-dd HH:mm:ss'),format(endTime,'yyyy-MM-dd HH:mm:ss'),false),
          style: 'cancel',
        },
        {text: 'Yes', onPress: () => updateCompetition(newTitle,newDescription,format(startTime,'yyyy-MM-dd HH:mm:ss'),format(endTime,'yyyy-MM-dd HH:mm:ss'),true)},
      ],{
        cancelable: true,
      });
    }
